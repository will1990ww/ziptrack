(function () {
  ...
  function box(item) {
    ...
    if (!item || item.type === "empty") { el.classList.add("empty"); el.innerHTML = '<div class="drop"><b>More coming soon</b><span>New installations added regularly</span></div>'; return el; }
    var cap = '<div class="mcap"><h4>' + (item.title || "") + '</h4>' + (item.meta ? '<p>' + item.meta + '</p>' : '') + '</div>';
    if (item.type === "video") {
      var poster = item.poster ? ' poster="' + item.poster + '"' : '';
      el.innerHTML = '<span class="mtag">Video</span><video preload="none"' + poster + ' playsinline src="' + item.src + '"></video><button class="play" type="button" aria-label="Play video: ' + (item.title || "") + '">' + PLAY + '</button>' + cap;
  ...
  Object.keys(DATA).forEach(function (cat) { var host = document.querySelector('[data-gallery="' + cat + '"]'); if (!host) return; DATA[cat].forEach(function (item) { host.appendChild(box(item)); }); });
})();
